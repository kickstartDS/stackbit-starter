...
import * as allModels from "../cms/models.json";
...
const allReferenceFields: { [key: string]: any } = {};
(allModels.components as Model[]).forEach((model) => {
...
type IndexableContentObject = types.ContentObject & {
  [key: string]: any;
};

function resolveReferences(
  content: IndexableContentObject,
  ...
  for (const fieldName in content) {
    ...
    if (Array.isArray(fieldValue)) {
      ...
      if (typeof fieldValue[0] === "object") {
        fieldValue.forEach((o: IndexableContentObject) =>
...
export function allContent(): types.ContentObject[] {
  let objects = contentFilesInPath(contentBaseDir).map((file) =>
    readContent(file)
  );
...
function annotateContentObject(o: any, prefix = "", depth = 0) {
  ...
  if (depth === 0) {
    if (o.__metadata?.id) {
      ...
      if (logAnnotations)
        console.warn("[annotateContentObject] NO object ID:", o);
    }
  } else {
